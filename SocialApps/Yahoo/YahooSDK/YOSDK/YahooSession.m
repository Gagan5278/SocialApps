...
- (void)sendUserToAuthorization
{
	...
	if (newRequestToken && newRequestToken.key && newRequestToken.secret) {
        ...
        self.authorizationWebView.backgroundColor=[UIColor lightGrayColor];
        ...
        buttonCancel.frame=CGRectMake(10, 20, 39, 18);
        [buttonCancel setImage:[UIImage imageNamed:@"cancel-btn"] forState:UIControlStateNormal];
        [buttonCancel addTarget:self action:@selector(CloseLoginView) forControlEvents:UIControlEventTouchUpInside];
        [self.authorizationWebView addSubview:buttonCancel];
        [self.authorizationWebView bringSubviewToFront:buttonCancel];
...
-(void)CloseLoginView
{
    [UIView transitionWithView:self.authorizationWebView duration:.1 options:UIViewAnimationOptionTransitionNone animations:^{
        self.authorizationWebView.alpha=0.5;
...
{
    self.authorizationWebView.backgroundColor=[UIColor whiteColor];
    [self removeActivityIndicator];
}

-(void)webViewDidStartLoad:(UIWebView *)webView
{
    if(![self.authorizationWebView.subviews containsObject:self.activityIndicator])
    {
       self.activityIndicator = [[JPRadialActivityIndicator alloc] initWithFrame:(CGRect){0,0,  (CGSize){80, 80}}];
        self.activityIndicator.center=CGPointMake([UIScreen mainScreen].bounds.size.width/2, [UIScreen mainScreen].bounds.size.height/2);
       self.activityIndicator.backingColor = [UIColor whiteColor];
       [self.authorizationWebView addSubview: self.activityIndicator];
       self.activityIndicator.hidesWhenStopped = YES;
       [ self.activityIndicator startAnimating];
    }
    [self.authorizationWebView bringSubviewToFront: self.activityIndicator];
}
